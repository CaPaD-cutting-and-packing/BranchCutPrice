...
StepS:                //:: The adding step
  ++ k;             dbgpr(5,"\nS: k="<<k<<" lb="<<lb);
	pc=&pieces[k];  dbgpr(5," bnd="<<sad+UBRest(pieces, k, dL));
  ub = UBRest(pieces, k, dL);     ///// (pc->qma)*(double)dL;
// 	cout << " UB = " << ub << ", old = " << (pc->qma)*(double)dL << ' ' << flush;
	if (ub<0) ub=0;
...
StepB2:                 dbgpr(5,"\n  B2:");
    ...
    if (0 == fmod( (++nSteps), 1024)) {
      if (nSteps > nStepsTooMuch) { // even if not found ?
				if (found) {
					log_n_(2,'.');
					log_n_(3, " BB Col Gen >" << nStepsTooMuch << " steps but feas " );
					fETerm = 1;
					goto Exit;
				}
				if (nSteps > nStepsWAYTOOMUCH) { // even if not found.
					log_ln(" \n\n  BB Col Gen has taken " << nSteps
					<< " steps (see settings [BB]) and not found any solution with neg red cost."
					" Switching to KP col gen and AFF branching (see README)\n\n");
					throw double(8.1);
				}
				if (!nPrintsThisRun) {
					++nPrintsThisRun;
					if (0 == (++nPRINTSTOOLONG) % 100)
						log_ln(" \n\n  BB Col Gen has taken >" << nStepsTooMuch 
						<< " steps (see settings [BB]) and not found any solution with neg red cost."
						" After " << nStepsWAYTOOMUCH << " steps (see BB.nSteps),"
						" automatic switching to KP col gen and AFF branching ---"
						" to do it manually, set:\n"
						" CSP1.fRootPricing=1, BCP.fLocalUB=0 and "
						"seem better BCP.fLocaLReduce=0 with Subgr.nIterMax=0. "
						" For the faster AFF branching, set BCP.nBranch=1, CSP1.nHP=1, "
						" and BCP1.maxCuts=0 and better CSP1.fModelEquality=1 (see README)\n\n" );
				}
			}
...
/*    << opt::MakeOpt(&fRandomizeWeights, true,
    ...
    << opt::MakeOpt(&nStepsTooMuch, 1e+6,
      "nStepsTooMuch",
      "So many backtrack steps and feas sol found => termination")
    << opt::MakeOpt(&nStepsWAYTOOMUCH, 2e+8,
										"nStepsWAYTOOMUCH",
      "so many backtrack steps => termination even if no good sol. found and switch to "
		" KP col gen & AFF branching (see README)")
